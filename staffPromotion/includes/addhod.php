<?php
	...
	if(isset($_POST['submit'])){
		...
		if($name == "" || $email == "" || $fac == "" || $dpt == "" || $level == "" || $sdate == "" || $password == ""){
			$error = urlencode("all fields are required");
			header("location: ../addHod.php?error=".$error);
			return false;
		}else{
			$name = trimData($name);
		...
		if ($_FILES['file']['name'] != '') {
			...
			if(in_array($fileactualext, $allow)){
				if($filesize < 8000000){
					...
					$filedestination = 'hoddp/'.$pic;
					
					if(move_uploaded_file($filetmp, $filedestination)){

						$sql = "INSERT INTO hod(name, email, password, dpt, fac, slvl, sdate, pdate, sphoto)VALUES('$name', '$email', '$password', '$dpt', '$fac', '$level', '$sdate', '$sdate', '$pic')";
						$result = mysqli_query($connect, $sql);
						if($result){
							$success = urlencode("New HOD was addede successfully...");
							header("location: ../addHod.php?success=".$success);
							return false;
						}else{
							$error = urlencode("error creating HOD");
							header("location: ../addHod.php?error=".$error);
							return false;
						}
					}else{
						$failed = urlencode("error uploading picture");
						header("location: ../addHod.php?error=".$failed);
						return false;
					}
				}else{
